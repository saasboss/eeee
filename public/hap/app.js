...
function adminHome(){
 ...
 ${done<tasks.length?`<section class="section"><div class="checklist" data-tour="checklist"><div class="checklist-head"><div><strong>Finish setting up</strong><span>${done} of ${tasks.length} done</span></div><div class="ring" style="--pct:${pct}"><span>${pct}%</span></div></div><div class="checklist-body">${tasks.map(t=>`<button class="check-row ${t.done?'done':''}" data-action="${t.done?'noop':t.action}" ${t.tab?`data-tab="${t.tab}"`:''} ${t.page?`data-page="${t.page}"`:''}><i class="check-box">${t.done?icon('check',12):''}</i><div><strong>${t.title}</strong><span>${t.sub}</span></div>${t.done?'':icon('chevron',15)}</button>`).join('')}</div></div></section>`:''}
